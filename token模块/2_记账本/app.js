...
const authRouter = require('./routes/web/reg')
const tokenRouter = require('./routes/api/auth')
...
app.use(session(
  {
      ...
      store: MongoStore.create({
          mongoUrl: `mongodb://${DBHOST}:${DBPOST}/${DATABASE}` //数据库的连接配置
      }),
      cookie: {
          httpOnly: true, //   开启后前端无法通过 JS 操作
          maxAge: 1000 * 60 * 60 * 24 * 7 // 这一条 是控制 sessionID 的过期时间的！！！  也是cookie的过期时间
      },
  }
  ))
//设置路由前缀 
